//mass scavenging by Sophie "Shinko to Kuma" (modified - add per-unit Keep/Send mode)
// serverTimeTemp, serverTime, serverDate etc (same as original)
...
if (game_data.locale == "el_GR") {
    ...
    langShinko = [
        ...
        "Î§ÏÏŒÎ½Î¿Ï‚",
        "Î¥Ï€Î¿Î»ÏŒÎ³Î¹ÏƒÎµ Ï‡ÏÏŒÎ½Î¿Ï‚ ÏƒÎ¬ÏÏ‰ÏƒÎ·Ï‚ Î³Î¹Î± ÎºÎ¬Î¸Îµ ÏƒÎµÎ»Î¯Î´Î±.",
        "Î”Î·Î¼Î¹Î¿Ï…ÏÎ³ÏŒÏ‚: ",
...
// NEW: sendAmount (how many to SEND per unit) - coexists with keepHome
if (localStorage.getItem("sendAmount") == null) {
    console.log("No sendAmount found, creating new one");
    var sendAmount = {
        "spear": 0,
        "sword": 0,
        "axe": 0,
    ...
    console.log("Grabbing sendAmount from storage");
    var sendAmount = JSON.parse(localStorage.getItem("sendAmount"));
}
...
for (var i = 0; i < sendOrder.length; i++) {
    ...
    // Build per-unit UI with Mode select and both inputs (Keep and Send)
    var unitName = sendOrder[i];
    var initialMode = (sendAmount && sendAmount[unitName] > 0) ? "send" : "keep";
    var keepVal = (keepHome && keepHome[unitName]) ? keepHome[unitName] : 0;
    var sendVal = (sendAmount && sendAmount[unitName]) ? sendAmount[unitName] : 0;

    var unitHtml = `<td align="center" style="background-color:${backgroundColor}">
    <table class="vis" border="1" style="width: 100%">
    <thead>
    </thead>
    <tbody>    
        <tr>
            <td style=" text-align:center;background-color:${headerColor};padding: 5px;"><img src="https://dsen.innogamescdn.com/asset/cf2959e7/graphic/unit/unit_${unitName}.png" title="${unitName}" alt="" class=""></td>
        </tr>
        <tr>
            <td align="center" style="background-color:${backgroundColor};padding: 5px;"><input type="checkbox" ID="${unitName}" name="${unitName}"></td>
        </tr>
        <tr>
            <td style="text-align:center; width:auto; background-color:#202225;padding: 5px;">
                <font color="#ffffdf">Mode</font><br>
                <select id="${unitName}Mode" style="width:80px;">
                    <option value="keep" ${initialMode === "keep" ? "selected" : ""}>Keep</option>
                    <option value="send" ${initialMode === "send" ? "selected" : ""}>Send</option>
                </select>
            </td>
        </tr>
        <tr>
            <td align="center" style="background-color:${backgroundColor};padding: 5px;">
                <input type="text" ID="${unitName}Backup" name="${unitName}" value="${keepVal}" size="5" placeholder="Keep" ${initialMode === "send" ? 'style="display:none;"' : ""}>
                <input type="text" ID="${unitName}Send" name="${unitName}" value="${sendVal}" size="5" placeholder="Send" ${initialMode === "keep" ? 'style="display:none;"' : ""}>
            </td>
        </tr>
        </tbody>  
    </table>
...
// After creating the unit UI, attach change listeners for the Mode dropdowns so fields toggle
for (var i = 0; i < sendOrder.length; i++) {
    (function(unit) {
        $(`#${unit}Mode`).on("change", function () {
            if (this.value === "keep") {
                $(`#${unit}Backup`).show();
                $(`#${unit}Send`).hide();
            } else {
...
function readyToSend() {
    ...
    // Save keepHome and sendAmount according to Mode dropdown
    for (var i = 0; i < sendOrder.length; i++) {
        var unit = sendOrder[i];
        var modeVal = $(`#${unit}Mode`).val();
        if (modeVal === "send") {
            // parseInt, fallback to 0
            sendAmount[unit] = parseInt($(`#${unit}Send`).val()) || 0;
            keepHome[unit] = 0;
        } else {
            keepHome[unit] = parseInt($(`#${unit}Backup`).val()) || 0;
            sendAmount[unit] = 0;
        }
    }
    ...
    localStorage.setItem("troopTypeEnabled", JSON.stringify(troopTypeEnabled));
    localStorage.setItem("keepHome", JSON.stringify(keepHome));
    localStorage.setItem("sendAmount", JSON.stringify(sendAmount)); // save new sendAmount
    ...
    if (data.has_rally_point == true) {
        ...
        for (key in troopTypeEnabled) {
            if (troopTypeEnabled[key] == true) {
                // NEW: If sendAmount is set (>0), use that as the number to send (bounded by available)
                // Otherwise, fall back to keepHome behaviour (available - keepHome)
                let available = data.unit_counts_home[key] || 0;
                if (sendAmount && sendAmount[key] > 0) {
                    troopsAllowed[key] = Math.max(0, Math.min(available, sendAmount[key]));
                } else {
                    if (available - (keepHome[key] || 0) > 0) {
                        troopsAllowed[key] = available - (keepHome[key] || 0);
                    } else {
                        troopsAllowed[key] = 0;
                    }
                }
...
function resetSettings() {
    ...
    localStorage.removeItem("sendAmount"); // clear sendAmount as well
